...
def optimize_circuit(params, hamiltonian):
    ...
    for _ in range(max_iter):
        params, old_cost = optimizer.step_and_cost(circuit, params, hamiltonian)
        params = params[0]
        new_cost = circuit(params, hamiltonian)
        if abs(new_cost - old_cost) < conv_tol:
            break
    return new_cost  # Return the value of the minimized QNode
